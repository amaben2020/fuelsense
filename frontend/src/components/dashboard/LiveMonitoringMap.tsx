...
export function LiveMonitoringMap({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const next: AnimatedTrack[] = targets.map(({ track, prev }) => ({
        ...track,
        displayLat: lerp(prev.lat, track.current.lat, eased),
        displayLng: lerp(prev.lng, track.current.lng, eased),
        displayHeading: lerp(prev.heading, track.heading, eased),
      }));
  ...
  const center = useMemo(() => {
    if (selectedTrack) {
      return { lat: selectedTrack.displayLat, lng: selectedTrack.displayLng };
    }
    if (animated.length === 0) return LAGOS;
    const lat = animated.reduce((s, t) => s + t.displayLat, 0) / animated.length;
    const lng = animated.reduce((s, t) => s + t.displayLng, 0) / animated.length;
    return { lat, lng };
  }, [animated, selectedTrack]);
    ...
    <div className="relative h-full min-h-[calc(100vh-8rem)] w-full overflow-hidden rounded-xl border border-[#434656]">
      <APIProvider apiKey={MAPS_KEY}>
        <Map
          ...
          colorScheme="DARK"
          center={center}
          zoom={selectedTrack ? 14 : animated.length <= 1 ? 13 : 12}
          tilt={45}
          heading={0}
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 bg-gradient-to-b from-[#0b1326]/90 to-transparent p-4">
        ...
        <p className="text-xs text-[#8e90a2]">
          {animated.length} vehicles on map · routes update every 3s
        </p>
      </div>
      ...
      {selectedTrack && (
        <div className="absolute right-4 top-16 w-64 rounded-xl border border-[#434656] bg-[#171f33]/95 p-4 backdrop-blur-md">
